...
class _TimelineChatSheetState extends State<TimelineChatSheet> {
  ...
  void initState() {
    ...
    context
        .read<ChatCubit>()
        .loadPreviousConversations(roomId: widget.orderId)
        .then((value) {
      _scrollToBottom();
      initSocketService();
      initChatEvents();
  ...
  @override
  void deactivate() {
    context.read<ChatCubit>().viewChat(widget.orderId);
    super.deactivate();
  }
  ...
  void initChatEvents() {
    socket.emit('joinRoom', widget.orderId);
    HelperFunc.logger('joinRoom: ${widget.orderId}');
    ...
    return Scaffold(
        body: SafeArea(
            ...
            child: Column(children: [
              ...
              Row(children: [
                ProfileAvatar(radius: 24, avatar: widget.receiverImg),
                HelperFunc.sb(8.w),
                Text(widget.receiverName,
                        style: AppTextStyles.mediumText(fontSize: 14))
                    .EXPANDED,
                GestureDetector(
                  onTap: () => HelperFunc.makePhoneCall(widget.receiverPhone),
                  child: CircleAvatar(
                      radius: 17.r,
                      ...
                      child: ListView.builder(
                          ...
                          itemBuilder: (context, i) {
                            return MessageCard(
                                ...
                                isSender:
                                    widget.riderId == state.chats[i].senderId,
...
class MessageCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
        ...
        children: [
          if (!isSender) ProfileAvatar(radius: 17.r),
          ...
          if (isSender)
            BlocBuilder<ProfileCubit, ProfileState>(builder: (context, state) {
              return ProfileAvatar(
                  radius: 17.r, avatar: state.user?.profileImg);
            })
        ]);
  }
}
